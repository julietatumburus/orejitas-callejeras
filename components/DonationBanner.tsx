...
export default function DonationBanner({ alias }: { alias: string }) {
  ...
  return (
    <div className="mt-10 rounded-3xl border border-pink-200 bg-gradient-to-r from-pink-100 to-rose-100 p-8 text-center shadow-sm">
      <p className="text-3xl">💗</p>
      <h3 className="mt-2 text-xl font-bold text-pink-700">Hacé tu donación</h3>
      ...
      <div className="mt-5 inline-flex flex-wrap items-center justify-center gap-3 rounded-full bg-white px-4 py-2 shadow-sm ring-1 ring-pink-200">
        ...
        <span className="font-mono text-base font-semibold text-pink-700">{alias}</span>
        <button
          type="button"
          onClick={copy}
          className="rounded-full bg-pink-500 px-4 py-1.5 text-sm font-semibold text-white transition hover:bg-pink-600"
        >
